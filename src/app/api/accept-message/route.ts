...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (!updatedUser) {
            return Response.json( {success: true, message: "failed to update user status to accept messages"}, {status: 401});    
        }

        return Response.json( {success: true, message: "Message acceptance status successfully", updatedUser}, {status: 200}); 
